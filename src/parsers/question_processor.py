...
class QuestionProcessor:
    ...
    def _process_correct_answer(self, body, assets_folder):
        ...
        answer_p = body.find('p', class_=lambda c: c and all(cls in c.split() for cls in ['card-text', 'question-answer']))
        if not answer_p:
            return markdown_lines
            ...
            if progress_bar:
                ...
                for bar in vote_bars:
                    ...
                    if bar.get('style') and 'display: flex' in bar.get('style'):
                        ...
                        width_match = re.search(r'width:\s*(\d+)%', bar.get('style', ''))
                        percentage = width_match.group(1) if width_match else "?"
                        
                        # Get vote count from tooltip
                        vote_count = "?"
                        if bar.has_attr('data-original-title') and 'vote' in bar['data-original-title']:
                            vote_count_match = re.search(r'(\d+)\s*vote', bar['data-original-title'])
                            if vote_count_match:
